...
class Canigen:
    ...
    def __init__(
        ...
        if obd_config_filename is not None:
            for ecu in self._obd_config["ecus"]:
                thread = Thread(
                    name=f"CanigenObd-{ecu['tx_id']}",
                    target=self._obd_thread,
                    args=(ecu,),
                )
                thread.start()
                self._threads.append(thread)
    ...
    def _obd_thread(self, ecu):
        ...
        while not self._stop:
            if create_socket:
                ...
                isotp_socket_phys = self._create_isotp_socket(
                    txid, rxid_phys, ecu["zero_padding"]
                )
                isotp_socket_func = self._create_isotp_socket(
                    txid, rxid_func, ecu["zero_padding"]
                )
            ...
            tx = [sid | 0x40]
            if (
                ecu.get("require_broadcast_requests", False)
                and res[0][0] != isotp_socket_func
            ):
