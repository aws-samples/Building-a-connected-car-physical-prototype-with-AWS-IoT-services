...
class Cloud():
  ...
  def set_local_value(self, shadow_property, value):
      ...
      payload = {shadow_property: value}
      print("Set local value")
      print(payload)
      r = requests.post("http://localhost:5000/setled", payload)
      ...
      if response.state:
        if response.state.delta:
          ...
          if value:
            ...
            self.change_shadow_value(shadow_property, value)
            return
        ...
        if response.state.reported:
          ...
          if value:
            ...
            self.set_local_value(shadow_property, response.state.reported[shadow_property])
            return
      ...
      for shadow_property in self.shadow_properties:
        ...
        if delta.state and (shadow_property in delta.state):
            ...
            if value is None:
                ...
                # sostituire con set_local_value
                if shadow_property == "RedLed":
                  property = 1
                else: 
                  property = 2
                payload = {"led": property, "status": str(value).lower() }
                print(payload)
                r = requests.post("http://localhost:500/setled", payload)  
      else:
