...
Note: to use Amazon Managed Grafana, you must have configured AWS IAM Identity Center and create a user. You can refer to the [documentation](https://docs.aws.amazon.com/SetUp/latest/UserGuide/setup-enableIdC.html).
...
In this step we will create the AWS Timestream resources and the S3 to store rejected records.

To create these resources you can use the aws-cli run the following command:
...
#### Step 3: Create AWS FleetWise Role for writing in Timestream (AWS CLI)

Create the TimestreamExecutionRole that will be used by AWS IoT Fleetwise campaign to write to Amazon Timestream table.
...
aws cloudformation create-stack \
--stack-name macchinetta-fleetwise-role \
--template-body file://03-FleetWise-Role.yml \
--capabilities CAPABILITY_NAMED_IAM
